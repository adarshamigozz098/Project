...
const increaseQuantity = async (req, res) => {
  try {
    ...
    const cartItem = await Cart.findOne({ user_id: userId }).populate(
      "items.product_id"
    );
    ...
    if (!cartItem) {
      return res
        .status(404)
        .json({ success: false, error: "Cart not found for the user" });
    }

    const itemIndex = cartItem.items.findIndex((item) =>
    ...
    Product.quantity =
      Product.quantity - (quantity - cartItem.items[itemIndex].quantity);
    await Product.save();
